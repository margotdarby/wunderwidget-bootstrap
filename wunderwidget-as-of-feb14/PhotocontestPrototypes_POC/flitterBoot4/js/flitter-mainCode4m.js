...
//Try replacing this 'keyed' flickr call with one to a public search, no key. Either getJSON or full-ajax code. Leave the 'self.add(response.photos.photo);' the same for the data return.
        ...
        FlickrImageView = Backbone.View.extend({
            tagName: 'a',
            template: _.template("<img src='<%= thumb_url() %>' /><p><strong>Show Title in Template</strong><br/>ID PE65000</p><div class='picpod-buttons'> <a href='response.html'><img class='thumbsdown' src='images/flag-thumbsdown.gif' /></a> <a href='response.html'><img class='thumbsup'  src='images/editorspick-thumbsup.gif' /></a>  <a href='response.html'><img class='keepfor' src='images/keepfornextround.gif' /></a>  </div>"),
/*          
            var picpod_wrap = {
                thumbURL: "img src='<%= thumb_url() %>'",
    			showTitle: "Show Title in Template",
    			showId: "ID PE65000",
    			picpodButtons: {
    			    responseURL: "a href='response.html'",
    				thumbsDown: "img src='images/flag-thumbsdown.gif'",
    				thumbsUp: "img src='images/editorspick-thumbsup.gif'",
    				keepFor: "img src='images/keepfornextround.gif'"
    				}
},
            var template = '{{thumbURL}}<p><strong>{{showTitle}}</strong><br/>{{showId}}</p>{{#picpodButtons}}{{#responseURL}}{{thumbsDown}}{{/responseURL}}{{#responseURL}}{{thumbsUp}}{{/responseURL}}{{#responseURL}}{{keepFor}}{{/responseURL}}{{/picpodButtons}}';
            var thisPhoto = Mustache.to_html(template, picpod_wrap);
            $('.photo').html(html);
